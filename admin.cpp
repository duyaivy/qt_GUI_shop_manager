...
void admin::setGroupBtn(){
    ...
    connect(buttonGroup, &QButtonGroup::buttonClicked, this, [=](QAbstractButton* button) {
        ...
        case 5:{
            setupProfilePage();
            setupProfileInvoice();
            displayNotifications(ui->notifyW);
        }
        default:{
            setUpInvoiceViewHome();
            break;
...
void admin::displayNotifications(QWidget* notifyWidget) {

        // Kiểm tra và xóa layout cũ nếu có
        if (notifyWidget->layout()) {
            delete notifyWidget->layout();  // Xóa layout cũ
        }

        // Tạo layout cho widget chứa thông báo
        QVBoxLayout* layout = new QVBoxLayout(notifyWidget);

        // Tạo một QWidget để chứa các thông báo (để gắn vào QScrollArea)
        QWidget* container = new QWidget();
        QVBoxLayout* containerLayout = new QVBoxLayout(container);

        // Duyệt qua vector noti và tạo widget cho từng thông báo
        int count = 0;
        for (Notify *n : Notify::noti) {
            if(n->getStatus()) continue;
            if(curEmployee->getID()!= n->getEmpID()) continue;

            QWidget* notification = new QWidget();
            notification->setStyleSheet("border: 1px solid lightgray; border-radius: 10px;");
            notification->setFixedHeight(120);

            // Tạo layout bên ngoài (chứa thông tin và nút)
            QHBoxLayout* mainLayout = new QHBoxLayout(notification);

            // Phần bên trái (thông tin hóa đơn)
            QVBoxLayout* infoLayout = new QVBoxLayout();
            QLabel* idLabel = new QLabel("Hóa Đơn " + QString::fromStdString(n->getInvoiceID()) );
            idLabel->setStyleSheet("color: #333;font-size: 18px;background-color: transparent; border:none; padding-left: 20px;");
            string name = Invoice::getInvoiceByID(n->getInvoiceID())->getCustomerID();
            QLabel* nameLabel = new QLabel("Yêu cầu xác nhận thanh toán từ "+ QString::fromStdString(name));
            nameLabel->setStyleSheet("color: #333;font-size: 18px;background-color: transparent; border:none; padding-left: 20px;");
            string moneyzz = formatCurrency(n->getPrice());
            QLabel* moneyLabel = new QLabel(QString::fromStdString(moneyzz));
            moneyLabel->setStyleSheet("font-weight: 700;font-size: 24px;background-color: transparent;color: #396AFF; border:none; padding-left: 20px;");

            infoLayout->addWidget(idLabel);
            infoLayout->addWidget(nameLabel);
            infoLayout->addWidget(moneyLabel);
            infoLayout->setSpacing(5);  // Khoảng cách giữa các label

            // Phần bên phải (nút Xác Nhận và Hủy)
            QVBoxLayout* buttonLayout = new QVBoxLayout();
            QPushButton* confirmButton = new QPushButton("Xác Nhận");
            confirmButton->setFixedSize(200, 50);
            confirmButton->setStyleSheet("background-color: blue;font-size: 18px; color: white; border-radius: 10px; margin-right: 20px;");
            connect(confirmButton, &QPushButton::clicked, [this, n]() {
                n->setStatus(1);
                Invoice *i =Invoice::getInvoiceByID(n->getInvoiceID());
                if(i){
                    i->setStatus(1);
                }
                 this->displayNotifications(ui->notifyW);
            });
            ...
            cancelButton->setStyleSheet("background-color: red;font-size: 18px; color: white; border-radius: 10px; margin-right: 20px;");
            connect(cancelButton, &QPushButton::clicked, [this, n]() {
                 n->setStatus(1);
                this->displayNotifications(ui->notifyW);
            ...
            buttonLayout->setSpacing(5);  // Khoảng cách giữa hai nút
            confirmButton->setCursor(Qt::PointingHandCursor);
            cancelButton->setCursor(Qt::PointingHandCursor);

            // Thêm phần thông tin và nút vào layout chính (QHBoxLayout)
            mainLayout->addLayout(infoLayout);
            mainLayout->addLayout(buttonLayout);
            mainLayout->setSpacing(10);  // Khoảng cách giữa thông tin và nút
            mainLayout->setAlignment(buttonLayout, Qt::AlignRight);
            // Thêm thông báo vào layout chính của container
            containerLayout->addWidget(notification);
            count ++;
        }
        if (count <= 3) {

            QSpacerItem* spacer = new QSpacerItem(20, 120, QSizePolicy::Minimum, QSizePolicy::Expanding);
            containerLayout->addItem(spacer);  // Thêm khoảng cách vào layout
        }
        // Đảm bảo các thông báo cuộn khi vượt quá kích thước của widget
        QScrollArea* scrollArea = new QScrollArea(notifyWidget);
        scrollArea->setWidget(container);
        scrollArea->setWidgetResizable(true);
        scrollArea->setStyleSheet("QScrollArea { border: none; }");

        // Thêm scrollArea vào layout của widget notifyWidget
        layout->addWidget(scrollArea);
        notifyWidget->setLayout(layout);
